import { a as getDefaultExportFromCjs, g as getAugmentedNamespace, c as commonjsGlobal } from "./_commonjsHelpers-BhWcALO8.js";
...
import { c as commonjsRequire } from "./index-B4ntsY6U.js";
import { app } from "electron";
import { AccountModel } from "./account-BYxVCDKs.js";
import { ProxyModel } from "./proxy-BFfub94j.js";
import { CodeModel } from "./code-jNMM3jF_.js";
import require$$0$3 from "worker_threads";
import require$$0$4 from "punycode";
import require$$0$7 from "node:util";
import require$$0$8 from "node:stream";
import require$$0$5 from "child_process";
import require$$0$6 from "node:child_process";
import require$$1$4 from "node:crypto";
...
const [isReadableStream, isRequest$1, isResponse, isHeaders] = ["ReadableStream", "Request", "Response", "Headers"].map(kindOfTest);
...
const inherits = (constructor2, superConstructor, props, descriptors2) => {
...
function requireFollowRedirects() {
  ...
  function wrap(protocols) {
    ...
    Object.keys(protocols).forEach(function(scheme) {
      ...
      function get2(input2, options, callback) {
  ...
  function parseUrl(input2) {
    ...
    if (useNativeURL) {
      ...
      parsed = validateUrl(url.parse(input2));
...
function requireUrlStateMachine() {
  ...
  (function(module) {
    ...
    function utf8PercentDecode(str) {
      const input2 = new Buffer(str);
      const output2 = [];
      for (let i = 0; i < input2.length; ++i) {
        if (input2[i] !== 37) {
          output2.push(input2[i]);
        } else if (input2[i] === 37 && isASCIIHex(input2[i + 1]) && isASCIIHex(input2[i + 2])) {
          output2.push(parseInt(input2.slice(i + 1, i + 3).toString(), 16));
          i += 2;
        } else {
          output2.push(input2[i]);
        }
  ...
  return new fetch$1.Promise(function(resolve, reject) {
    ...
    req.on("response", function(res) {
      ...
      if (codings == "gzip" || codings == "x-gzip") {
        body = body.pipe(zlib.createGunzip(zlibOptions2));
        response = new Response(body, response_options);
        resolve(response);
        return;
      }
...
function requireCreateWorker() {
  ...
  let workerCounter = 0;
  createWorker = async (langs = "eng", oem = OEM2.LSTM_ONLY, _options = {}, config2 = {}) => {
    ...
    const recognize = async (image, opts = {}, output2 = {
      text: true
    }, jobId) => startJob(createJob2({
      id: jobId,
      action: "recognize",
      payload: { image: await loadImage2(image), options: opts, output: output2 }
    }));
...
function requireIs() {
  ...
  const inRange = (val, min2, max2) => val >= min2 && val <= max2;
...
function requireResize() {
  ...
  function extend2(extend3) {
    if (is2.integer(extend3) && extend3 > 0) {
    ...
    } else if (is2.object(extend3)) {
      if (is2.defined(extend3.top)) {
        if (is2.integer(extend3.top) && extend3.top >= 0) {
          this.options.extendTop = extend3.top;
        } else {
          throw is2.invalidParameterError("top", "positive integer", extend3.top);
        }
      }
      if (is2.defined(extend3.bottom)) {
        if (is2.integer(extend3.bottom) && extend3.bottom >= 0) {
          this.options.extendBottom = extend3.bottom;
        } else {
          throw is2.invalidParameterError("bottom", "positive integer", extend3.bottom);
        }
      }
      if (is2.defined(extend3.left)) {
        if (is2.integer(extend3.left) && extend3.left >= 0) {
          this.options.extendLeft = extend3.left;
        } else {
          throw is2.invalidParameterError("left", "positive integer", extend3.left);
        }
      }
      if (is2.defined(extend3.right)) {
        if (is2.integer(extend3.right) && extend3.right >= 0) {
          this.options.extendRight = extend3.right;
        } else {
          throw is2.invalidParameterError("right", "positive integer", extend3.right);
        }
      }
      this._setBackgroundColourOption("extendBackground", extend3.background);
      if (is2.defined(extend3.extendWith)) {
        if (is2.string(extendWith[extend3.extendWith])) {
          this.options.extendWith = extendWith[extend3.extendWith];
        } else {
          throw is2.invalidParameterError("extendWith", "one of: background, copy, repeat, mirror", extend3.extendWith);
        }
      }
    } else {
      throw is2.invalidParameterError("extend", "integer or object", extend3);
    }
    return this;
  }
...
function requireColor() {
  ...
  var conversions_default = convert2;
  var LAB_FT = (6 / 29) ** 3;
  ...
  convert2.rgb.oklab = function(rgb) {
  ...
  convert2.xyz.oklab = function(xyz) {
    const x = xyz[0] / 100;
    const y = xyz[1] / 100;
    const z = xyz[2] / 100;
  ...
  convert2.oklab.oklch = function(oklab) {
    return convert2.lab.lch(oklab);
  };
  convert2.oklab.xyz = function(oklab) {
  ...
  convert2.oklab.rgb = function(oklab) {
    const ll = oklab[0] / 100;
  ...
  convert2.oklch.oklab = function(oklch) {
    return convert2.lch.lab(oklch);
  };
  ...
  for (const fromModel of models) {
    convert22[fromModel] = {};
    Object.defineProperty(convert22[fromModel], "channels", { value: conversions_default[fromModel].channels });
    Object.defineProperty(convert22[fromModel], "labels", { value: conversions_default[fromModel].labels });
    const routes = route_default(fromModel);
    const routeModels = Object.keys(routes);
    for (const toModel of routeModels) {
      const fn = routes[toModel];
      convert22[fromModel][toModel] = wrapRounded(fn);
      convert22[fromModel][toModel].raw = wrapRaw(fn);
    }
  }
  var color_convert_default = convert22;
  var skippedModels = [
...
async function preprocessImageToWhiteBg(imagePath) {
  ...
  require$$0$1.writeFileSync(imagePath, buffer);
}
...
async function solveCaptchaForTet(token) {
  ...
  if (captchaData.value && captchaData.images) {
    ...
    const recaptcha2 = serverRes?.result || serverRes?.data?.result;
    ...
    return { recaptcha: recaptcha2, raw: { captchaRes, serverRes } };
  }
  ...
  const imagePath = require$$1$2.join(userDataPath, "captcha.png");
  let base64Data = imageBase64;
  if (base64Data.startsWith("data:image")) {
    base64Data = base64Data.split(",")[1];
  }
  const buf = Buffer.from(base64Data, "base64");
  require$$0$1.writeFileSync(imagePath, buf);
...
async function startExchangeGift({ accountCollectionId, proxyCollectionId, codeCollectionId, timeoutMs }) {
  if (taskRunning) {
