import { a as getDefaultExportFromCjs } from "./_commonjsHelpers-HSQAuv0d.js";
...
function initDatabase() {
  ...
  const dbPath = require$$1.join(userDataPath, "data.db");
...
function runMigrations() {
      ...
      up: () => {
        ...
        if (tableInfo) {
          ...
          if (!columnNames.includes("token")) {
            db.exec(`ALTER TABLE accounts ADD COLUMN token TEXT NOT NULL DEFAULT ''`);
  ...
  for (const migration of migrations) {
    const executed = db.prepare("SELECT * FROM migrations WHERE name = ?").get(migration.name);
    if (!executed) {
